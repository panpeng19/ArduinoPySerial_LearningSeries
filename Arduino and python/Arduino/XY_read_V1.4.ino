...
uint32_t local_buff[6],running_sum[6];
uint16_t average[6];
...
{ //if (!(t_ms & 0x3ff)) {Serial.print("b");}
 if (!(t_ms & 0x3ff)) {float V=Volts*(int16_t) average[button_selected];Serial.print(button_selected);Serial.print(":");Serial.print(average[button_selected], BIN);Serial.print(" ");
                       Serial.print((int16_t) average[button_selected]);Serial.print(" ");Serial.print(V,3);Serial.println(" Volts ");}
  #ifdef LEDKEY
  if (!(t_ms & 0x0f)) {Display_int_LEDKEY(average[button_selected]);} //every 16ms {Display_int_LEDKEY(t_ms);}
  //if (!(t_ms & 0x03ff)) 
...
{ uint32_t dummy,*ptr=local_buff;
  ...
  if(!BUFFERS_FULL)      // enter only if we have room to place data, our 16 bit data are all in port C which has 32 bits
      {CS_ADC1_low;       // first thing to do is put CS low (High when we leave)
       ...
       //if (!(nombre_appels & 0x1))// every 512
           for (uint32_t i = 0; i < 6; i++){average[i]=uint16_t (running_sum[i]);running_sum[i]=0;} 
